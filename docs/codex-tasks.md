...
Codex Tasks 是 Codex 内置的任务管理系统，用于在软件开发过程中追踪、组织和管理复杂的开发工作。它支持跨会话持久化、依赖关系管理和并发安全操作。
...
Tasks 不会在会话结束后丢失。所有任务数据存储在 `~/.codex/tasks/` 中，新会话开始时自动加载。
...
```
~/.codex/tasks/
└── {project-id}/                    ← 项目标识（通常为 base64url 路径 ID）
    └── {session-id}/                ← 会话 ID（threadId）
        ├── 1.json                   ← Task ID 1
        ├── 2.json                   ← Task ID 2
        ├── 3.json                   ← Task ID 3
        ├── 4.json                   ← Task ID 4
...
Codex Tasks 是一个轻量级但功能完整的任务管理系统，设计用于：
